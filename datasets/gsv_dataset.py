...
class GSVDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print("gsv_220",gt_zvp)
        if gt_zvp[2] < 0:
            gt_zvp = -gt_zvp
        #print("gsv_dataset___222_____,",gt_zvp)
        gt_zvp = gt_zvp / np.maximum(gt_zvp[2], 1e-7)
        #print("gsv_dataset___224_____,",gt_zvp)
        gt_zvp = gt_focal * gt_zvp
        #print("gsv_dataset___227_____,",gt_zvp)
        gt_zvp[2] = 1.0
        #print("gsv229",gt_zvp)
        ...
        gt_hvp = gt_hvps.copy()
        #print("gsv293______",gt_hvps)
        gt_hvps[:, 0] = rho * (gt_hvps[:, 0] - pp[0])
        gt_hvps[:, 1] = rho * (gt_hvps[:, 1] - pp[1])
        #print("gsv242______",gt_hvps)
        gt_hvp[:, 0] = rho * (gt_hvp[:, 0] - pp[0])
        gt_hvp[:, 1] = rho * (gt_hvp[:, 1] - pp[1])

        # 수정 compute horizon line1 & 2
        # hp1 = gt_hvps[0, :]
        # hp2 = np.array([pp[0] * rho, pp[1] * rho, 1])
        # hp3 = gt_hvps[1, :]

        # gt_horizon_lines1 = np.cross(hp1, hp2)
        # gt_horizon_lines2 = np.cross(hp3, hp2)

        # gt_horizon_lines1 = normalize_safe_np(gt_horizon_lines1)
        # gt_horizon_lines2 = normalize_safe_np(gt_horizon_lines2)
        ...
        gt_vp = np.array([gt_zvp,gt_hvp1,gt_hvp2])

        #print("gsv261________",gt_hvp1)
        if self.return_masks:
            masks = create_masks(image)

        image = np.ascontiguousarray(image)
        target["rp"] = (
            torch.from_numpy(np.ascontiguousarray(gt_rp)).contiguous().float()
        )
        target["fovy"] = (
            torch.from_numpy(np.ascontiguousarray(gt_fovy)).contiguous().float()
        )
        target["up_vector"] = (
            torch.from_numpy(np.ascontiguousarray(gt_up_vector)).contiguous().float()
        )
        target["focal"] = (
            torch.from_numpy(np.ascontiguousarray(gt_focal)).contiguous().float()
        )
        target["vp1"] = (
            torch.from_numpy(np.ascontiguousarray(gt_zvp)).contiguous().float()
        )
        target["vp2"] = (
            torch.from_numpy(np.ascontiguousarray(gt_hvp1)).contiguous().float()
        )
        target["vp3"] = (
            torch.from_numpy(np.ascontiguousarray(gt_hvp2)).contiguous().float()
        )
        target["vp"] = (
            torch.from_numpy(np.ascontiguousarray(gt_vp)).contiguous().float()
        )
        target["hl"] = (
            torch.from_numpy(np.ascontiguousarray(gt_hl)).contiguous().float()
        )
        target["hvps"] = (
...
def build_gsv(image_set, cfg):
    root = "/home/kmuvcl/source/modified_CTRL-C/google_street_view_191210/manhattan/"
