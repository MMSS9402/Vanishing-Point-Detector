...
def main(cfg):
    ...
    dataset_test = build_su3_dataset(image_set='test', cfg=cfg)
    ...
    checkpoint = torch.load('/home/kmuvcl/CTRL-C/su3_100+/checkpoint0047.pth', map_location='cpu')
    ...
    for i, (samples, extra_samples, targets) in enumerate(tqdm(data_loader_test)):
        with torch.no_grad():
            ...
            if cfg.TEST.DISPLAY:
                ...
                #plt.axis('off')
                try:
                    plt.savefig(osp.join(fig_output_dir, filename[30:]+'.jpg'),  
                            pad_inches=0, bbox_inches='tight')
                except FileNotFoundError:
                    os.makedirs(osp.join(fig_output_dir, filename[30:33]),exist_ok=True)
                plt.close('all')
                ...
                plt.ylim(-256/focal_length, 256/focal_length)
                #plt.axis('off')
                plt.savefig(osp.join(fig_output_dir, filename[30:]+'_vp1_tgt_src.jpg'),  
                            pad_inches=0, bbox_inches='tight')
                plt.close('all')
                ...
                #plt.axis("off")
                plt.savefig(
                    osp.join(fig_output_dir, filename[30:] + "_lines_segment.jpg"),
                    pad_inches=0,
                    bbox_inches="tight",
                )
                ...
                #plt.axis("off")
                plt.savefig(
                    osp.join(fig_output_dir, filename[30:] + "pred_lines_class.jpg"),
                    pad_inches=0,
                    bbox_inches="tight",
                )
                plt.close("all")
                ...
                #plt.axis("off")
                plt.savefig(
                    osp.join(fig_output_dir, filename[30:]+ "mask_class.jpg"),
                    pad_inches=0,
                    bbox_inches="tight",
                )
                plt.close("all")
